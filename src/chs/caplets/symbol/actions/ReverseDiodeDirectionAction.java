...
/**
 * Created by IntelliJ IDEA. User: nagamani Date: Feb 16, 2010 Time: 3:14:43 PM To change this template use File |
 * Settings | File Templates.
 */
public class ReverseDiodeDirectionAction extends ControllerActionRT implements ICtxMenuProvider
{

	Set<ISchemInternalLink> m_linksToEdit;

	public ReverseDiodeDirectionAction(ICapletController controller)
	{
		super(controller);
	}
			...
			if (selectionCount / 2 == selections.getSelectCount(schemInternalLinksFilter)) {
				while (selectedObjIter.hasNext()) {
					...
					if (obj instanceof ISchemInternalLink) {
						...
						if (linkType.equalsIgnoreCase("Diode")) {
	...
	{
		return ReverseDiodeDirectionActionUI.class.getName();
	}
