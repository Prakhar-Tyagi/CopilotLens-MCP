...
/**
 * Created by IntelliJ IDEA. User: nagamani Date: Feb 16, 2010 Time: 7:25:48 PM To change this template use File |
 * Settings | File Templates.
 */
public class ConvertToResistanceLinkAction extends ControllerActionRT implements ICtxMenuProvider
{

	Set<ISchemInternalLink> m_linksToEdit;

	public ConvertToResistanceLinkAction(ICapletController controller)
	{
		super(controller);
	}
	...
	protected boolean onTerminate(boolean successful)
	{
		for (ISchemInternalLink link : m_linksToEdit) {
			link.getConnectivity().setLinktype("Resistance");
			...
			if (selectionCount / 2 == selections.getSelectCount(schemInternalLinksFilter)) {
				while (selectedObjIter.hasNext()) {
					...
					if (obj instanceof ISchemInternalLink) {
						...
						if (linkType.equalsIgnoreCase("Diode") || linkType.equalsIgnoreCase("Fusing")) {
	...
	{
		return ConvertToResistanceLinkActionUI.class.getName();
	}
