 ...
 * @author rharring
 */
public class QualitativeSimulationModeAction extends ControllerActionRT
{

	protected static final String modeChangedToQualString =
			ResourceMgr.getString(QualitativeSimulationModeAction.class,
					"QualitativeSimulationModeAction.String.modeChangedToQual");
	private static final String ELECTRICAL_QUALITATIVE = "electrical.qualitative";

	/**
	 * Creates a new instance of SimulateAction
	 */
	public QualitativeSimulationModeAction(ICapletController c)
	{
		super(c);
	...
	{
		return QualitativeSimulationModeActionUI.class.getName();
	}

	public String getStatusbarText()
	{
		return ResourceMgr.getString(QualitativeSimulationModeAction.class,
				"QualitativeSimulationModeAction.String.statusbarText");
	}
	...
	protected IActionEnum onActivate(ActionEvent e)
	{
		...
		LogicAnalysisServices.getAnalysisServices().getControlPanel().setMessage(modeChangedToQualString);
		...
		LogicAnalysisServices.getAnalysisServices().setSimulationType(ELECTRICAL_QUALITATIVE);
		...
		if (session != null) {
			...
			session.setSimulationType(uid, ELECTRICAL_QUALITATIVE, StringUtils.getBytes(netlist));
			if (LogicAnalysisServices.getCurrentAnalysisNetlistScope() != null) {
	...
	public boolean isSetSimulationMode()
	{
		...
		//System.err.println("In isSetSimulationMode QSME" );
		IAnalysisSimulationSessionController session = LogicAnalysisServices.getAnalysisServices().getSimSession(uid);
		String target = "electrical.qualitative";
		if (session != null) {
		...
		// we return true here for any case when there is no session or there is no
		// set sim type as this is the default.
		return "electrical.qualitative".equals(target);
	}
